            ...
            match (entity, event_entry.event_type.as_str()) {
                (MeasuredEntity::TdvfKernel, "EV_EFI_BOOT_SERVICES_APPLICATION") => {
                    let raw_bytes = &event_entry.event_desc[KERNEL_VENMEDIA_DEVPATH_OFFSET
                        ..KERNEL_VENMEDIA_DEVPATH_OFFSET + 2 * entity.to_string().len()];

                    match read_string(raw_bytes) {
                        Ok(kernel) => {
                            if kernel == entity.to_string() {
                                return Some(hex::encode(&event_entry.digests[0].digest));
                            }
                ...
                (MeasuredEntity::TdvfKernelParams | MeasuredEntity::TdvfInitrd, "EV_EVENT_TAG") => {
                    ...
                    match String::from_utf8(
                        ...
                        Ok(event) => {
                            if event == entity.to_string() {
                                return Some(hex::encode(&event_entry.digests[0].digest));
                            }
                            warn!(
                                "Event {event:?} did not match with {:?}",
                                entity.to_string()
                            );
                        }
                ...
                (me, ev) => trace!(
                    "Unmatched MeasuredEntity {:?} with event {ev}",
                    me.to_string()
                ),
            }
        }
