...
use openssl::x509::{self, store::X509StoreBuilder, X509StoreContext, X509VerifyResult, X509};
...
fn parse_certification_data(data: &[u8]) -> Result<QuoteSignatureData> {
    ...
    let r = BigNum::from_slice(&quote_signature.sig_r)?;
    let s = BigNum::from_slice(&quote_signature.sig_s)?;
    let _ecdsa_sig = EcdsaSig::from_private_components(r, s)?;

    let x = BigNum::from_slice(&quote_signature.pkey_x_coord)?;
    let y = BigNum::from_slice(&quote_signature.pkey_y_coord)?;
    let group = EcGroup::from_curve_name(Nid::X9_62_PRIME256V1)?;
    let _pub_key = EcKey::from_public_key_affine_coordinates(&group, &x, &y)?;
        ...
        [5, 0] => {
            ...
            match r#type {
                ...
                QuoteV5Type::TDX15 => {
                    ...
                    if qe.qe_certification_data.certificates[1]
