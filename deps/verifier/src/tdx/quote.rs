...
fn parse_certification_data_v4(data: &[u8]) -> scroll::Result<QuoteSignatureData> {
    let offset = &mut 0;
...
fn cert_data_start(q: &Quote) -> usize {
    match q {
...
mod tests {
    ...
    async fn test_parse_tdx_quote(#[case] quote_path: &str) {
        ...
        let mut start = cert_data_start(&quote);

        let cert_len: u32 = quote_bin.pread::<u32>(start).expect("MUST succeed");
        start += std::mem::size_of::<u32>();

        let end = start + cert_len as usize;
        assert_eq!(
            quote_bin.len(),
            end,
